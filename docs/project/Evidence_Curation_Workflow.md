...
| **Gespeicherte Provenienz** | `duplicate_of`. Keine Zyklen zulässig, `duplicate_of` (und die gesamte Kette verketteter Duplikate) muss innerhalb desselben Protokolls bleiben (siehe `tools/validate_research.py`). |
