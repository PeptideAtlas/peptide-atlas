...
!!! info "Vollständige Deduplizierungsarchitektur seit Phase 4B-1B-2 (ADR-0058)"
    ...
    bereits ein Screening Record für den Ziel-Kandidaten, muss dessen `related_records[]` den passenden
    INVERSEN Typ zurücktragen (fehlend: Warnung; falsch: Fehler). Der Bearbeitungszustand eines Kandidaten
    (`system_initialized`/`under_human_review`/`finalized`) ist eine reine, nicht gespeicherte Projektion
    aus `decision_history` (`tools/_researchlib.py::derive_workflow_state()`) — kein Schemafeld. Siehe
