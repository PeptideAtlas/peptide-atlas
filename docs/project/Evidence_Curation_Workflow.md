...
| **Erforderliche Prüfungen** | Abgleich normalisierter DOI/PMID/PMCID/NCT-ID/ISBN (`tools/_researchlib.py`, wiederverwendet aus bzw. analog zu `tools/_datalib.py::normalize_*`) — von `tools/validate_research.py` tatsächlich durchgesetzt: zwei aktive (nicht `duplicate`-markierte) Kandidaten mit identischem normalisiertem Identifikator **innerhalb desselben Protokolls** sind ein Validierungsfehler, es sei denn, die gesamte Kollisionsgruppe ist bereits als eine einzige Zusammenhangskomponente erklärt (seit ADR-0058, Phase 4B-1B-2: Union-Find über `duplicate_of`- UND NUR VOLLSTÄNDIG VALIDIERTEN `related_records`-Kanten — eine einseitig dokumentierte oder falsch-inverse Beziehung verbindet die Gruppe nicht, verschärft in CSO-Review Runde 3; nicht mehr nur eine einfache Zählung aktiver Mitglieder). Kollisionen über verschiedene Protokolle hinweg sind erlaubt; URL-Kollisionen lösen nur eine Warnung aus. Seit ADR-0057 (verschärft/verallgemeinert in ADR-0058): eine noch nicht vollständig verbundene Kollisionsgruppe, an der mindestens ein Mitglied noch den Bearbeitungszustand `system_initialized` hat (`tools/_researchlib.py::derive_workflow_state()`), ist nur eine Warnung — sobald kein Mitglied mehr `system_initialized` ist, wird eine weiterhin ungelöste Kollision zum Fehler. Zwei getrennte Auflösungswege: bibliographische Dubletten über `decision: duplicate`/`duplicate_of`, oder eigenständige, aber inhaltlich verwandte Publikationen (z. B. Letter+Reply) über ein vollständig validiertes `related_records[]`-Beziehungspaar (siehe unten) — auch eine Kombination beider Mechanismen zählt. |
...
!!! info "Vollständige Deduplizierungsarchitektur seit Phase 4B-1B-2 (ADR-0058)"
    `related_records[]` dokumentiert eigenständige, aber inhaltlich verwandte Kandidaten (z. B. Letter+Reply,
    Preprint+publizierte Fassung) — strukturell getrennt von `duplicate_of`, das ausschließlich für
    bibliographische Dubletten (derselbe Text) reserviert bleibt. Referenziert Kandidaten
    (`related_candidate_manifest_id`/`related_candidate_id`), nicht Screening Records. `relationship_type`
    ist ein gerichtetes 17-wertiges Vokabular (9 Konzeptpaare plus `other_related_to` als symmetrische
    Ausnahme); jeder Eintrag trägt eine Pflicht-`rationale` sowie `relationship_metadata`
    (`identified_by`/`identified_at`/`evidence_source[]`) — **ausschließlich Evidenz dafür, dass die
    Beziehung besteht, niemals die wissenschaftliche Evidenz der beteiligten Studie(n) selbst.** Existiert
    bereits ein Screening Record für den Ziel-Kandidaten, ist dessen `related_records[]`-Gegenstück mit dem
    passenden INVERSEN Typ **Pflicht** — fehlend oder falsch typisiert ist ein **Fehler** (verschärft in
    CSO-Review Runde 3). Eine **Warnung** gilt ausschließlich, solange für den Ziel-Kandidaten noch kein
    Screening Record existiert — und nur eine solche, noch nicht existierende Gegenrichtung kann keine
    Kollisionsgruppe verbinden; eine einseitig dokumentierte oder falsch-inverse Beziehung zwischen zwei
    bereits existierenden Screening Records zählt ebenfalls NICHT als Kollisionskante. Der
    Bearbeitungszustand eines Kandidaten (`system_initialized`/`under_human_review`/`finalized`) ist eine
    reine, nicht gespeicherte Projektion aus `decision_history` (`tools/_researchlib.py::
    derive_workflow_state()`) — kein Schemafeld. Siehe
    [Scientific Research Protocol](Scientific_Research_Protocol.md), Abschnitt 9f für die vollständige
    Regel.
...
| **Erforderliche Prüfungen** | Ist `final` in `dual_reviewer_stages` aufgeführt, ist `second_review` mit einer expliziten, eigenständigen `reviewer_decision` Pflicht, `second_review.reviewed_by` ≠ `screened_by` (Reviewer-Unabhängigkeit) — beides von `tools/validate_research.py` erzwungen. Seit ADR-0059 (Phase 4B-1B-3, verschärft in CSO-Review Runde 2) zusätzlich **unabhängig von `dual_reviewer_stages` und unabhängig von der konkreten Entscheidung**: ist `screened_by`/`decided_by` in `research/reviewers/**` als `ai_assistant`/`automation` registriert, ist `second_review` für jede nicht-administrative primäre wissenschaftliche Entscheidung Pflicht. `reviewer_decision` selbst muss ebenfalls zur Stage-/Decision-Matrix passen (an `final` z. B. kein `pending`/`duplicate`/`awaiting_full_text`, ADR-0046). `full_text_status: obtained` bleibt Voraussetzung für ein terminales `include`. Eine Adjudikation (siehe Abbruchkriterien) muss auf einen **registrierten** `human`-Akteur als `resolved_by` verweisen — ein unregistriertes Kürzel ist hier ungültig. |
...
!!! info "Reviewer-Modell und Wiederaufnahme seit Phase 4B-1B-3 (ADR-0059)"
    ...
    Zweitreview-Pflicht (für jede nicht-administrative primäre wissenschaftliche Entscheidung, nicht nur
    `include`/`exclude`); Adjudikation erfordert in jedem Fall einen **registrierten** `human`-Akteur — ein
    unregistriertes Kürzel ist hier, anders als bei normalen Erst-/Zweitprüfern, ungültig. Unabhängig davon
    kann ein neuer `decision_history[]`-Eintrag an derselben Stufe eine frühere, bereits **settled**
    Entscheidung umkehren (mechanisch bereits möglich, echter Rückwärtslauf über Stufen hinweg bleibt
    verboten) — `revision_context` (`reason`/`reference`/`triggered_by`, Vokabular
    `research/vocabularies/screening_revision_reasons.yaml`) macht diese Wiederaufnahme semantisch explizit
    und ist in diesem Fall Pflicht; `triggered_by` muss ebenfalls ein **registrierter** `human`-Akteur sein.
    Siehe [Scientific Research Protocol](Scientific_Research_Protocol.md), Abschnitte 9a/9e für die
    vollständige Regel.
